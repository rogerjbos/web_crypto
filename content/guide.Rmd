...
After to cteate your validator, if you decide to make any changes to it, you can edit the parameters.  Here is an example of adding a website url to the validator description.
```
chain-maind tx staking edit-validator --keyring-backend="test" \
  --moniker="sagepoint-node" \
  --website="https://crypto.rogerjbos.com" \
  --from=Default \
  --fees=5000basetcro \
  --chain-id="testnet-croeseid-2" 
```

View validator signing information
In order to keep track of a validator's signatures in the past you can do so by using the signing-info command:
```
./chain-maind query slashing signing-info \
  tcrocnclconspub1zcjduepqr37wjtjgqgjshlz5p7z4yzq7t64yslqx07rvcxn832n79n3rzxfqmt7asj \
  --chain-id="testnet-croeseid-2" 
```
...
Check the signing info of your validator, again using the pubkey address of your validator
```
chain-maind q slashing signing-info tcrocnclconspub1zcjduepqr37wjtjgqgjshlz5p7z4yzq7t64yslqx07rvcxn832n79n3rzxfqmt7asj
```
The most interesting outout is shown in the `missed_blocks_counter` field which shows how many blocks were missed out of the last 2000.  If your validator misses over 1000 blocks of the last 2000, it will be jailed for at least one hour.  Once it is jailed, you will have to issue an unjail transaction to free it (after the first hour is up).  You will also need to insure your validator is fully synced up before trying to unjail.
...
Somtimes you will get an error `Internal error: timed out waiting for tx to be included in a block`.  You can minimize that by increasing the timeout period.  Modify `/.chain-maind/config/config.toml` to change the `timeout_broadcast_tx_commit` from 10 seconds to 20 seconds.
```
